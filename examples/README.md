...
dfx deploy backend --argument '(
  record {
    expiration_minute = 60 : nat64;
    canisters = vec {};
    authorities = vec {
      principal "your-authorized-principal";
    };
  },
)'

# Deploy to IC mainnet
dfx deploy --network ic backend --argument '(
  record {
    expiration_minute = 60 : nat64;
    canisters = vec {};
    authorities = vec {
 ...
 */
async function authenticateWithSIWT(
  telegramUserId,
  targetCanisters = [],
  authorizedIdentity,
) {
...
const authorizedIdentity =
  Ed25519KeyIdentity.fromSecretKey(/* your authorized key */);
